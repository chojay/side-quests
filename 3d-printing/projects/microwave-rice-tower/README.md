...
**Compatibility:** sized for the round Korean-style bowl format, so it takes most of what US warehouse and H-Mart shelves carry: CJ Hetbahn (햇반, 137 mm), Ottogi-bap, and Bibigo including the [Costco 7.4 oz twelve-count](https://www.costco.com/p/-/bibigo-cooked-sticky-white-rice-bowls-medium-grain-74-oz-12-count/4000040390). It does **not** take rectangular trays, the format many Japanese brands use - square corners cannot ride a round cage, and a rectangular version would need a different retention geometry, not a resize.

**Why:** CJ publishes no container dimensions. The sizing came from a camping hobbyist's hand measurements of five brands; the tower is deliberately sized to the widest bowl of the set (139 mm Ottogi-bap), which is what buys the multi-brand fit above. Full story in [../../playbooks/rice-tower-case-study.md](../../playbooks/rice-tower-case-study.md).
...
**Files:** `rice_tower_v1.py` (first version) and `rice_tower_v6.py` (final, printed). Run either with build123d + trimesh installed; STL and 3MF are regenerated next to the script, so no meshes are checked in. v6 cut filament 23% (258 to 199 cm3) through an ordered lightening pass done only after the design was liked.
