...
Skills that expect private local setup (a vault path, a sibling tool, a browser extension) carry a clearly marked "Adaptation notes" section saying exactly what to supply; placeholders like `<vault>/` mark where to point them at your own system. The self-contained ones (academic-svg, arxiv-pdf, pdf-to-svg-converter) need only their declared pip dependencies. Sanitization for public release is documented in the notes rather than hidden.
...
| [arxiv-pdf](arxiv-pdf/) | Markdown with YAML frontmatter to arXiv-style preprint PDF (or .tex for actual submission) via pandoc and a bundled LaTeX template; see the [worked example](arxiv-pdf/examples/) |
